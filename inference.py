...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None: #ghost eaten
            allPossible[self.getJailPosition()] = 1
        else:
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0: #observation update
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # make new counter object to not interfere with update in for loop
        ...
        beliefDis.normalize(); #normalize
        self.beliefs = beliefDis;
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while self.numParticles > len(self.particles):
            for var in self.legalPositions:
                #append variable to list
                self.particles.append(var)
    ...
    def observe(self, observation, gameState):
        ...
        #if ghost is captured, mark all jail positions 1
        if noisyDistance is None:
            self.beliefs[self.getJailPosition()] = 1
        else:
            #update beliefs based on the given distance observation
            for pos in self.particles:
                self.beliefs[pos] += emissionModel[util.manhattanDistance(pos, pacmanPosition)];
    ...
    def elapseTime(self, gameState):
        ...
        for particle in range(self.numParticles):
            self.particles[particle] = util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, self.particles[particle])))
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # return convert to Counter object
        beliefDis = util.Counter()
        
        for particle in self.particles: 
            beliefDis[particle] += 1;
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        #permutations for each legal ghost position
        ghostList = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(ghostList)

        #store particles in a list
        while self.numParticles > len(self.particles):
            self.particles += ghostList
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            ...
            self.beliefs[particle] += probability
